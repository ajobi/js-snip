  ...
  describe('Complex Scenarios', () => {
    ...
    it('Is able to change text (CSS)', () => {
      cy.get('[data-cy=paragraph]').then(($paragraph) => {
        ...
        const newText =
          'This is some custom text that is only useful for this test case. The text should now be visible on the page and properly clamped to exactly two lines. Not more, not less. This text is completely useless for the world.'
        paragraph.textContent = newText

        snip(paragraph, { lines: 2, method: 'css' })
        expect(getLines(paragraph)).to.equal(2)
        expect(paragraph.textContent).to.equal(newText)
      })
    })
    //
    ...
    //       const paragraph = $paragraph.get()[0]
    //
    //       snip(paragraph, { lines: 1, method: 'js' })
